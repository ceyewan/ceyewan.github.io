...
- **工具（Tools）**：用于让智能体执行实时操作（如网页搜索、API 调用等），需显式调用，动作型。
- **知识源（Knowledge Sources）**：为智能体提供可检索、可推理的被动参考上下文，隐式访问，推理型。

**设计原则：**

- "要让智能体做事，用工具；要让智能体知道什么，用知识源。"
...
### 4.3 字符串知识源（String Knowledge Source）

适合小型结构化/非结构化知识，如公司政策、FAQ 等。可直接作为 `StringKnowledgeSource` 传入，便于快速搭建与测试。

### 4.4 知识源的作用范围

- **Agent 级别**：仅指定智能体可访问该知识（如产品分析师独享产品摘要）。
- **Crew 级别**：团队所有智能体共享知识（如市场、客服、销售均可访问产品资料）。

**设计建议：**  
如现实团队中有"信息隔离"需求，则用 Agent 级别；如需共享，则用 Crew 级别。

### 4.5 自定义知识源（Custom Knowledge Sources）

通过继承 `BaseKnowledgeSource`，可将任意 API、数据库、内部系统数据作为知识源。例如接入天气 API，构建实时天气问答智能体。  
**实现要点：**

- 重写 `load_content()` 方法，实现数据抓取与文本化。
- 支持自动分块（chunking）、嵌入与检索。

### 4.6 分块策略（Chunking）

- 大型文档需分块处理，防止单条嵌入过长导致检索失效。
- 推荐设置块间重叠，避免语义割裂。
- 详见"5 种 RAG 分块策略"专题。
...
- 默认使用与 LLM 同源的嵌入模型（如 OpenAI、Ollama）。
- 可自定义嵌入模型，实现本地化、隐私保护或成本优化（如用 Ollama 的 nomic-embed-text 本地嵌入）。
