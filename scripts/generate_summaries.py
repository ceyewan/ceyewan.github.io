...
# 给 LLM 的指令 (Prompt)
PROMPT_TEMPLATE = """
请你为一个博客文章生成一个简洁、吸引人的摘要。这个摘要将用于 SEO 和文章预览。
摘要应该：
1.  长度在 60 到 100 个汉字之间。
2.  用中文书写。
3.  准确概括文章的核心内容。
4.  避免使用我、我们等第一人称。
...
def generate_summary_with_llm(content: str) -> str:
    ...
    try:
        client = OpenAI(
            base_url="https://dashscope.aliyuncs.com/compatible-mode/v1",  # 替换为 ALYUN BAILIAN 的 API 基础 URL
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.7,
            max_tokens=1000, # 限制最大 token 数，以控制成本和长度
        )
